...
int main(int argc, const char *const *argv) {
  ...
  if (argc < 2) {
    ...
    cout << "Look the code for discovering another thousand of unuseful "
            "parameters"
         << endl;
    return -1;
  }
  ...
  // double range, double sigma, int kernsize, double lopt, double aopt, int
  // iterations
  scanmatcher.setMatchingParameters(maxUrange, maxrange, sigma, kernelSize,
                                    lstep, astep, iterations,
                                    computeCovariance);
