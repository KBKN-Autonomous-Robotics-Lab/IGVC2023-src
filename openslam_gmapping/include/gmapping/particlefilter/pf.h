...
void normalizeWeights(WeightVector &weights, unsigned int size,
                      double minWeight) {
...
void repeatIndexes(Vector &dest, const std::vector<int> &indexes,
  ...
  =======*/
  // assert(indexes.size()==particles.size()); //DIEGO non ne vedo il senso,
  // anzi è sbagliata dest.resize(particles.size());	    //      è sbagliato
  // anche questo
...
template <typename Vector>
void repeatIndexes(Vector &dest, const std::vector<int> &indexes2,
                   const Vector &particles, const std::vector<int> &indexes) {
  //	assert(indexes.size()==indexes2.size());
  dest = particles;
  unsigned int i = 0;
  for (std::vector<int>::const_iterator it = indexes2.begin();
       it != indexes2.end(); ++it) {
