...
indexResampler<Numeric>::resample(const vector<Numeric> > &weights) const {
  Numeric cweight = 0;

  // compute the cumulative weights
  unsigned int n = 0;
  for (vector<Numeric>::const_iterator it = weights.begin();
       it != weights.end(); ++it) {
    cweight += *it;
    n++;
  }
...
void evolver<State, Numeric, EvolutionModel>::evolve(
    std::vector<evolver::Particle> &dest,
    const std::vector<evolver::Particle> &src) const {
  dest.clear();
  for (std::vector<Particle>::const_iterator it = src.begin(); it != src.end();
       ++it)
    dest.push_back(evolutionModel.evolve(*it));
}

template <class State, class Numeric, class QualificationModel,
...
struct auxiliaryEvolver {
  ...
  void auxiliaryEvolver<State, Numeric, QualificationModel, EvolutionModel,
                        LikelyhoodModel>::
      evolve(std::vector<auxiliaryEvolver::Particle> &particles) {
    ...
    std::vector<unsigned int> indexes(
        indexResampler.resample(observationWeights));
    for (std::vector<unsigned int>::const_iterator it = indexes.begin();
         it != indexes.end(); it++) {
      Particle &particle = particles[*it];
      particle = evolutionModel.evolve(particle);
      particle.weight *=
          lykelyhoodModel.lykelyhood(particle) / observationWeights[*it];
    }
  }

  void
  auxiliaryEvolver<State, Numeric, QualificationModel, EvolutionModel,
                   LikelyhoodModel>::evolve(std::vector<Particle> &dest,
                                            const std::vector<Particle> &src) {
    dest.clear();
    std::vector<Numeric> observationWeights(particles.size());
    unsigned int i = 0;
    for (std::vector<Particle>::const_iterator it = src.begin();
         it != src.end(); ++it, i++) {
      observationWeights[i] =
          likelyhoodModel.likelyhood(qualificationModel.evolve(*it));
    }
    ...
    for (std::vector<unsigned int>::const_iterator it = indexes.begin();
      ...
      dest.back().weight *=
          likelyhoodModel.lykelyhood(particle) / observationWeights[*it];
    }
    return dest();
  }
