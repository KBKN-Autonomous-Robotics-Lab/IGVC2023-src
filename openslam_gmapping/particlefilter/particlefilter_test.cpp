...
struct LikelyhoodModel {
  double likelyhood(const Particle &p) const {
    double v = 1. / (0.1 + 10 * (p.p - 2) * (p.p - 2)) +
               0.5 / (0.1 + 10 * (p.p - 8) * (p.p - 8));
    return v;
  }
};
...
int main(unsigned int argc, const char *const *argv) {
  ...
  auxiliary_evolver<Particle, double, QualificationModel, EvolutionModel,
                    LikelyhoodModel>
      auxevolver;
  ...
  while (1) {
    ...
    for (vector<Particle>::iterator it = sirparticles.begin();
         it != sirparticles.end(); it++) {
    ...
    for (vector<Particle>::iterator it = auxparticles.begin();
         it != auxparticles.end(); it++) {
      it->setWeight(likelyhoodModel.likelyhood(*it));
    }
    os.open("aux.dat");
