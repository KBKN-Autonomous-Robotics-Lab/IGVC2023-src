...
GridSlamProcessor::TNodeVector GridSlamProcessor::getTrajectories() const {
  ...
  for (ParticleVector::const_iterator it = m_particles.begin();
    ...
    if (newnode->parent) {
      ...
      // cerr << __func__ << ": node " << newnode->parent << " flag=" <<
  ...
  while (!border.empty()) {
    // cerr << __func__ << ": border.size(PREPROCESS)=" << border.size() <<
    // endl; cerr << __func__ << ": parentCache.size(PREPROCESS)=" <<
    ...
    // cerr << __func__ << ": parentCache.size(POSTERASE)=" <<
    // parentCache.size() << endl;
