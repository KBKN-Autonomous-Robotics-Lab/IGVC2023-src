...
if you want to look for a specific topic search for one of the following
keywords in the file comments
...
int main(int argc, const char *const *argv) {
  ...
  processor->setMatchingParameters(maxUrange, maxrange, sigma, kernelSize,
  ...
  while (running) {
    while (CarmenWrapper::getReading(rr)) {
      ...
      // this returns true when the algorithm effectively processes (the
      // traveled path since the last processing is over a given threshold)
      if (processed) {
        cerr << "PROCESSED" << endl;
        ...
        const GridSlamProcessor::ParticleVector &particles =
            processor->getParticles();
        // remember to use a const reference, otherwise it copys the whole
        // particles and maps

        // this is for recovering the tree of PARTICLE TRAJECTORIES (obtaining
        // the ancestor of each particle)
        ...
        /*
                                        //then if you want to access the BEST
           MAP,
                                        //of course by copying it in a plain
           structure Map<double, DoubleArray2D, false>* mymap =
           processor->getParticles()[best_idx].map.toDoubleMap();
                                        //at this point mymap is yours. Can do
           what you want.

                                        double
           best_weight=particles[best_idx].weightSum; cerr << "Best Particle is
           " << best_idx << " with weight " << best_weight << endl;
