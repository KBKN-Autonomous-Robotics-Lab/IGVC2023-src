...
int CarmenWrapper::registerLocalizationMessages() {
  ...
  /*
    ...
    err = IPC_subscribe(CARMEN_LOCALIZE_MAP_QUERY_NAME, map_query_handler,
    NULL); carmen_test_ipc(err, "Could not subscribe",
    CARMEN_LOCALIZE_MAP_QUERY_NAME);
...
bool CarmenWrapper::start(const char *name) {
  ...
  carmen_simulator_subscribe_truepos_message(
      NULL, (carmen_handler_t)simulator_truepos_handler,
      CARMEN_SUBSCRIBE_LATEST);
...
bool CarmenWrapper::getReading(RangeReading &reading) {
  ...
  if (!m_rangeDeque.empty()) {
    //		cerr << __func__ << ": queue size=" <<m_rangeDeque.size() <<
    //endl;
  ...
  //	cerr << "fetch. elements= "<< m_rangeDeque.size() << " sval=" << sval
  //<<endl;
  pthread_mutex_unlock(&m_mutex);
  return present;
}
...
void CarmenWrapper::addReading(RangeReading &reading) {
  ...
  int sval;
  sem_getvalue(&m_dequeSem, &sval);
  //	cerr << "post. elements= "<< m_rangeDeque.size() << " sval=" << sval
  //<<endl;
}

// RangeSensor::RangeSensor(std::string name, unsigned int beams_num, unsigned
// int res, const OrientedPoint& position, double span, double maxrange):

void CarmenWrapper::robot_frontlaser_handler(
    carmen_robot_laser_message *frontlaser) {
  /*	if (! m_rangeSensor){
                  double res=0;
                  if (frontlaser->num_readings==180 ||
     frontlaser->num_readings==181) res=M_PI/180; if
     (frontlaser->num_readings==360 || frontlaser->num_readings==361)
                          res=M_PI/360;
                  assert(res>0);
                  m_rangeSensor=new
     RangeSensor("FLASER",frontlaser->num_readings, res, OrientedPoint(0,0,0),
     0, 89.9); m_sensorMap.insert(make_pair(string("FLASER"), m_rangeSensor));
          ...
          for (unsigned int i=0; i< (unsigned int)frontlaser->num_readings;
     i++){ reading[i]=(double)frontlaser->range[i];
          }
          reading.setPose(OrientedPoint(frontlaser->x, frontlaser->y,
     frontlaser->theta));
  */
...
RangeReading
CarmenWrapper::carmen2reading(const carmen_robot_laser_message &msg) {
  ...
  if (msg.laser_pose.theta == msg.robot_pose.theta && !m_frontLaser) {
    ...
    OrientedPoint rpose(msg.robot_pose.x, msg.robot_pose.y,
                        msg.robot_pose.theta);
    OrientedPoint lpose(msg.laser_pose.x, msg.laser_pose.y,
                        msg.laser_pose.theta);
    OrientedPoint dp = absoluteDifference(lpose, rpose);
    m_frontLaser = new RangeSensor(
        sensorName, msg.num_readings, res,
    ...
    cout << __func__ << ": " << sensorName << " configured."
         << " Readings " << m_frontLaser->beams().size() << " Resolution "
         << res << endl;
  }
  if (msg.laser_pose.theta != msg.robot_pose.theta && !m_rearLaser) {
    ...
    cout << __func__ << ": " << sensorName << " configured."
         << " Readings " << m_rearLaser->beams().size() << " Resolution " << res
         << endl;
  }

  const RangeSensor *rs = (msg.laser_pose.theta == msg.robot_pose.theta)
                              ? m_frontLaser
                              : m_rearLaser;
  ...
  reading.setPose(
      OrientedPoint(msg.robot_pose.x, msg.robot_pose.y, msg.robot_pose.theta));
  return reading;
}
...
carmen_robot_laser_message
CarmenWrapper::reading2carmen(const RangeReading &reading) {
  carmen_robot_laser_message frontlaser;
  ...
  frontlaser.laser_pose.theta = frontlaser.robot_pose.theta =
