...
void QGraphPainter::paintEvent(QPaintEvent *) {
  ...
  if (autoscale) {
    for (unsigned int i = 0; i < (unsigned int)width() && i < values.size();
         i++) {
  ...
  if (m_useYReference) {
    ...
    painter.drawLine(
        xoffset + boundary / 2, height() - (int)(scale * (reference - _min)),
  ...
  for (unsigned int i = 1;
       i < (unsigned int)width() - 2 * boundary - xoffset && i < values.size();
       i++) {
