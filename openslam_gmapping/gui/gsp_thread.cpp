...
int GridSlamProcessorThread::init(int argc, const char *const *argv) {
  ...
  if (configfilename.length() > 0) {
    ...
    resampleThreshold =
        cfg.value("gfs", "resampleThreshold", resampleThreshold);
    ...
    linearOdometryReliability = cfg.value("gfs", "linearOdometryReliability",
                                          linearOdometryReliability);
    angularOdometryReliability = cfg.value("gfs", "angularOdometryReliability",
                                           angularOdometryReliability);
    ebuf = (std::string)cfg.value("gfs", "estrategy", ebuf);
    considerOdometryCovariance = cfg.value("gfs", "considerOdometryCovariance",
...
GridSlamProcessorThread::~GridSlamProcessorThread() {
  ...
  for (deque<Event *>::const_iterator it = eventBuffer.begin();
       it != eventBuffer.end(); it++)
    delete *it;
}
...
void *GridSlamProcessorThread::fastslamthread(GridSlamProcessorThread *gpt) {
  ...
  gpt->setMatchingParameters(
      gpt->maxUrange, gpt->maxrange, gpt->sigma, gpt->kernelSize, gpt->lstep,
  ...
  if (gpt->infoStream()) {
    gpt->infoStream() << " initialPose=" << initialPose.x << " "
                      << initialPose.y << " " << initialPose.theta << cout
                      << " xmin=" << xmin << " ymin=" << ymin
                      << " xmax=" << xmax << " ymax=" << ymax << endl;
  }
  gpt->setMotionModelParameters(gpt->srr, gpt->srt, gpt->str, gpt->stt);
  gpt->setUpdateDistances(gpt->linearUpdate, gpt->angularUpdate,
                          gpt->resampleThreshold);
  gpt->setgenerateMap(gpt->generateMap);
  gpt->GridSlamProcessor::init(gpt->particles, xmin, ymin, xmax, ymax,
                               gpt->delta, initialPose);
...
#undef printParam
  ...
  if (!gpt->infoStream()) {
    cerr << "cant open info stream for writing by unuseful debug messages"
         << endl;
  } else {
    ...
    while (*(gpt->input) && gpt->running) {
      ...
      if (rr && gpt->running) {
        const RangeSensor *rs =
            dynamic_cast<const RangeSensor *>(rr->getSensor());
        assert(rs && rs->beams().size() == rr->size());

        bool processed = gpt->processScan(*rr);
        rawpath << rr->getPose().x << " " << rr->getPose().y << " "
                << rr->getPose().theta << endl;
        if (0 && processed) {
          cerr << "Retrieving state .. ";
          TNodeVector trajetories = gpt->getTrajectories();
          cerr << "Done" << endl;
          cerr << "Deleting Tree state .. ";
          for (TNodeVector::iterator it = trajetories.begin();
               it != trajetories.end(); it++)
            delete *it;
          cerr << "Done" << endl;
        }
        // 				if (0 && processed){
        // 					cerr << "generating copy" <<
        // endl;; 					GridSlamProcessor* m_gsp=gpt->clone(); 					Map<double,
        // DoubleArray2D, false>*
        // pmap=m_gsp->getParticles()[0].map.toDoubleMap() ; 					cerr << "deleting"
        // << endl; 					delete m_gsp; 					delete pmap;
        // 				}
      }
  ...
  int pnumber = 0;
  for (TNodeVector::iterator it = trajetories.begin(); it != trajetories.end();
       it++) {
    char buf[10];
  ...
  DoneEvent *done = new DoneEvent;
  gpt->addEvent(done);
  gpt->infoStream() << "Hallo, I am the gsp thread. I have finished. Do you "
                       "think it is the case of checking for unlocked mutexes."
                    << endl;
  return 0;
}
...
void GridSlamProcessorThread::start() {
  if (running)
    return;
  running = true;
  pthread_create(&gfs_thread, 0, (void *(*)(void *))fastslamthread,
                 (void *)this);
}
...
void GridSlamProcessorThread::onOdometryUpdate() {
  ...
  hypotheses.clear();
  weightSums.clear();
  for (GridSlamProcessor::ParticleVector::const_iterator part =
           getParticles().begin();
...
OrientedPoint GridSlamProcessorThread::boundingBox(SensorLog *log, double &xmin,
