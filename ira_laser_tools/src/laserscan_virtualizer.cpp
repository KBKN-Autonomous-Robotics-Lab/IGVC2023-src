...
public:
  ...
  void pointcloud_to_laserscan(Eigen::MatrixXf points,
                               pcl::PCLHeader scan_header, int pub_index);
...
void LaserscanVirtualizer::reconfigureCallback(
    laserscan_virtualizerConfig &config, uint32_t level) {
...
void LaserscanVirtualizer::virtual_laser_scan_parser() {
  ...
  for (int i = 0; i < tokens.size(); i++) {
    ...
    if (tfListener_.waitForTransform(
            base_frame, tokens[i], ros::Time(0),
            ros::Duration(1.0))) // Check if TF knows the transform from this
                                 // frame reference to base_frame reference
    {
      cout << "Elapsed: " << ros::Time::now() - beg << endl;
      cout << "Adding: " << tokens[i] << endl;
      tmp_output_frames.push_back(tokens[i]);
    } else
      cout << "Can't transform: '" << tokens[i] + "' to '" << base_frame << "'"
           << endl;
  }
  ...
  if ((tmp_output_frames.size() != output_frames.size()) ||
      !equal(tmp_output_frames.begin(), tmp_output_frames.end(),
             output_frames.begin())) {
    ...
    if (output_frames.size() > 0) {
      ...
      for (int i = 0; i < output_frames.size(); ++i) {
        if (output_laser_topic.empty()) {
          virtual_scan_publishers[i] = node_.advertise<sensor_msgs::LaserScan>(
              output_frames[i].c_str(), 1);
          cout << "\t\t" << output_frames[i] << " on topic "
               << output_frames[i].c_str() << endl;
        } else {
          virtual_scan_publishers[i] = node_.advertise<sensor_msgs::LaserScan>(
              output_laser_topic.c_str(), 1);
          cout << "\t\t" << output_frames[i] << " on topic "
               << output_laser_topic.c_str() << endl;
        }
      }
    } else
...
LaserscanVirtualizer::LaserscanVirtualizer() {
  ...
  point_cloud_subscriber_ = node_.subscribe<sensor_msgs::PointCloud2>(
      cloud_topic.c_str(), 1,
      boost::bind(&LaserscanVirtualizer::pointCloudCallback, this, _1));
  cloud_frame = "";
}
...
void LaserscanVirtualizer::pointCloudCallback(
  ...
  if (cloud_frame.empty()) {
    ...
    for (int i = 0; i < output_frames.size(); i++)
      if (tfListener_.waitForTransform(output_frames[i], cloud_frame,
                                       ros::Time(0), ros::Duration(2.0)))
        tfListener_.lookupTransform(output_frames[i], cloud_frame, ros::Time(0),
                                    transform_[i]);
  }
  ...
  for (int i = 0; i < output_frames.size(); i++) {
    ...
    // Initialize the header of the temporary pointcloud, needed to
    // rototranslate the three points that define our plane It shall be equal to
    // the input point cloud's one, changing only the 'frame_id'
    ...
    // Extract the points close to the z=0 plane, convert them into a laser-scan
...
void LaserscanVirtualizer::pointcloud_to_laserscan(
    Eigen::MatrixXf points, pcl::PCLHeader scan_header,
...
int main(int argc, char **argv) {
  ...
  f = boost::bind(&LaserscanVirtualizer::reconfigureCallback, &_laser_merger,
                  _1, _2);
