...
class TandemManager:
    def __init__(self):
        ...
        self.costmap_client1 = dynamic_reconfigure.client.Client(
            "/move_base/global_costmap/obstacle_layer1"
        )
        self.costmap_client2 = dynamic_reconfigure.client.Client(
    ...
    def waypoint_num_callback(self, msg):
        if (self.no_tandem_area) or (msg.data == self.waypoint_num) or (self.stop):
            return
    ...
    def laserscan_callback(self, msg):
        try:
            ...
            if (not self.stop) and (min_range < self.danger_dist):
                ...
                self.stop = False
                rospy.loginfo(
                    "Resumed navigation because the obstacle ahead was more than {}m away.".format(
                        self.danger_dist + 0.1
                    )
                )
