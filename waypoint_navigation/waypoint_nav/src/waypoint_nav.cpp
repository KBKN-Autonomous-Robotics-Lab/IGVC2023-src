...
public:
  ...
  */
  WaypointsNavigation()
      : has_activate_(false), stopped_(false),
        move_base_action_("move_base", true), rate_(10), last_moved_time_(0),
        dist_err_(1.0), min_dist_err_(0.3), min_yaw_err_(0.3) {
    ...
    start_server_ = nh.advertiseService(
        "start_wp_nav", &WaypointsNavigation::startNavigationCallback, this);
    ...
    resume_server_ = nh.advertiseService(
        "resume_nav", &WaypointsNavigation::resumeNavigationCallback, this);
    cmd_vel_sub_ =
        nh.subscribe("cmd_vel", 1, &WaypointsNavigation::cmdVelCallback, this);
    scan_sub_ =
        nh.subscribe("scan", 1, &WaypointsNavigation::laserscan_callback, this);
...
// Read finish_pose
  ...
  */
  void computeWpOrientation() {
    for (std::vector<geometry_msgs::Pose>::iterator it =
  ...
  */
  bool startNavigationCallback(std_srvs::Trigger::Request &request,
    ...
    if (StartFromTheMiddle) {
      ...
      while (compare_waypoint_ != waypoints_.poses.end() - 1) {
        i++;
        square_x = (compare_waypoint_->position.x - init_x) *
                   (compare_waypoint_->position.x - init_x);
        square_y = (compare_waypoint_->position.y - init_y) *
                   (compare_waypoint_->position.y - init_y);
        dist_w = sqrt(square_x + square_y);
        if (i == 1) {
          dir_w = atan2(compare_waypoint_->position.y,
                        compare_waypoint_->position.x);
          convert_x =
              (compare_waypoint_->position.x - init_x) * cos(-init_yaw) -
              (compare_waypoint_->position.y - init_y) * sin(-init_yaw);
          ...
          dir_w = atan2(compare_waypoint_->position.y - pre_waypoint_y,
                        compare_waypoint_->position.x - pre_waypoint_x);
          convert_x =
              (compare_waypoint_->position.x - init_x) * cos(-init_yaw) -
              (compare_waypoint_->position.y - init_y) * sin(-init_yaw);
          if (abs(dir_w - init_yaw) < (90 / 180.0 * M_PI) && convert_x > 0) {
    ...
    if (min_dist_w == std::numeric_limits<int>::max()) {
      ROS_WARN("Could not find the closest appropriate waypoint. Please check "
               "the direction of the robot again.");
      response.success = false;
      return false;
    } else {
  ...
  ++++++++++
  */
  bool resumeNavigationCallback(std_srvs::Trigger::Request &request,
                                std_srvs::Trigger::Response &response) {
    if (has_activate_) {
  ...
  */
  bool stopNavigationCallback(std_srvs::Trigger::Request &request,
                              std_srvs::Trigger::Response &response) {
    if (has_activate_) {
  ...
  */
  void cmdVelCallback(const geometry_msgs::Twist &msg) {
    if (has_activate_ && msg.linear.x > -0.001 && msg.linear.x < 0.001 &&
  ...
  */
  void sendNoObstacleGoal(float first_thresh, float after_second_thresh,
                          float goal_thresh, const geometry_msgs::Pose &dest) {
    if (laserscan.ranges.size() > 0) {
      ...
      float r =
          std::sqrt(std::pow(dest.position.x - robot_pos.getOrigin().x(), 2) +
                    std::pow(dest.position.y - robot_pos.getOrigin().y(), 2));
      int j = 0;
      int sum_num = 0;
      bool pass_bool = false;
      for (int i = 0; i < laserscan.ranges.size(); i++) {
        if (!(laserscan.ranges[i] < laserscan.range_min ||
              laserscan.ranges[i] > laserscan.range_max ||
              std::isnan(laserscan.ranges[i]))) {
          float theta =
              laserscan.angle_min + i * laserscan.angle_increment + robot_yaw;
      ...
      for (int i = 0;; i++) {
        ...
        for (int ii = 0; ii < j; ii++) {
          dist_waypoint[ii] = std::sqrt(std::pow(x[ii] - waypoint_x_update, 2) +
                                        std::pow(y[ii] - waypoint_y_update, 2));
        ...
        if ((i == 0 && min > first_thresh) ||
            (i == 0 &&
             std::sqrt(std::pow(waypoint_x_update, 2) +
                       std::pow(waypoint_y_update, 2)) < first_thresh)) {
          pass_bool = true;
          ...
          ROS_WARN("Waypoint x: %f, y: %f", waypoint_x_update,
                   waypoint_y_update);
        ...
        waypoint_x_update = (dest.position.x - robot_pos.getOrigin().x()) *
                            (r - 0.5 * (i + 1)) / r;
        waypoint_y_update = (dest.position.y - robot_pos.getOrigin().y()) *
                            (r - 0.5 * (i + 1)) / r;
      }
      if (!pass_bool) {
        std_srvs::Empty empty;
        ...
        ROS_WARN("Send goal x: %f, y: %f", waypoint_x_update,
                 waypoint_y_update);
  ...
  */
  bool onNavigationPoint(const geometry_msgs::Point &dest,
                         double dist_err = 1.0) {
    ...
    if (waypoint_list_[wp_num_.data - 1].stop) {
      ...
      if ((dist < min_dist_err_) && (diff_yaw < min_yaw_err_))
        return true;
      else if ((dist < min_dist_err_ * 2) && navigationFinished())
        return true;
      return false;
    }
  ...
  */
  tf::StampedTransform getRobotPosGL() {
    ...
    try {
      tf_listener_.lookupTransform(world_frame_, robot_frame_, ros::Time(0.0),
                                   robot_gl);
  ...
  */
  void startNavigationGL(const geometry_msgs::Pose &dest) {
    ...
    sleep();
    if (waypoint_list_[wp_num_.data - 1].stop) {
      tf::Quaternion q(dest.orientation.x, dest.orientation.y,
                       dest.orientation.z, dest.orientation.w);
      tf::Matrix3x3 m(q);
      double r, p, y;
      m.getRPY(r, p, y);
      target_yaw_ = y;
  ...
  */
  void run() {
    ...
    while (ros::ok()) {
      ...
      ROS_INFO_STREAM("x: " << waypoint_list_[wp_num_.data - 1].x
                            << ", y: " << waypoint_list_[wp_num_.data - 1].y);
      startNavigationGL(*current_waypoint_);
      resend_goal = 0;
      start_nav_time = ros::Time::now().toSec();
