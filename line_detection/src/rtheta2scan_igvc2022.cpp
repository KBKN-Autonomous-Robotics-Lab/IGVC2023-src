...
public:
  RthetaToLaserScan() : it_(nh_) {
    image_sub_ =
        it_.subscribe("/rtheta_img", 1, &RthetaToLaserScan::imageCallback,
                      this); // size (85 x 360)
    scan_pub_ = nh_.advertise<sensor_msgs::LaserScan>("/camera_scan", 50);
  }

  ~RthetaToLaserScan() { cv::destroyAllWindows; }
  ...
  void imageCallback(const sensor_msgs::ImageConstPtr &msg) {
    ...
    camera_scan.angle_increment =
        -(camera_scan.angle_max - camera_scan.angle_min) / num_readings;
    ...
    float r2d = 0.063; // multiplication value for convert to real distance from
                       // pixel distance
    // float r2d = 0.09;
    unsigned char *ptr;
    ...
    for (int i = 0; i < max_distance; i++) {
      ...
      for (int j = 90; j < num_readings + 90;
           j++) { // to adjust to order the LaserScan message, start col 90
        pixval = ptr[j % 360];
        if (pixval != 0) {
