...
public:
  // Constructor
  LineDetector() : it_(nh_) {
    image_sub_ =
        it_.subscribe("/cv_camera/image_raw", 1, &LineDetector::imageCallback,
                      this); // size (85 x 360)
    ...
    cv::Size S;
    if (sim_) {
      K = (cv::Mat_<float>(3, 3) << 197.33531703864534, 0.0, 399.5746325526872,
           0.0, 197.33723610911758, 399.51648288464474, 0.0, 0.0, 1.0);
      D = (cv::Mat_<float>(1, 4) << 0.08515002883616878, 0.012936405125198296,
           -0.0018582939071174716, 0.0008693014693496443);
      S = cv::Size(800, 800);
    } else {
      K = (cv::Mat_<float>(3, 3) << 472.4101671967401, 0.5799913144184203,
           750.6027871515556, 0.0, 472.753090371875, 753.8213756085806, 0.0,
           0.0, 1.0);
      D = (cv::Mat_<float>(1, 4) << -0.05367925701562622, 0.07140332123124533,
           -0.08089849852274485, 0.03184938555925065);
      S = cv::Size(1504, 1504);
    }
    cv::fisheye::initUndistortRectifyMap(K, D, cv::Mat::eye(3, 3, CV_32F), K, S,
  ...
  // Deconstructor
  ~LineDetector() { cv::destroyAllWindows(); }
  ...
  void imageCallback(const sensor_msgs::ImageConstPtr &msg) {
    ...
    // Undistortion & Resize
    cv::Mat undis;
    cv::remap(gray_img, undis, map1_, map2_, cv::INTER_LINEAR,
              cv::BORDER_CONSTANT);
  ...
  void reconfigCallback(line_detection::reconfigConfig &config,
                        uint32_t level) {
