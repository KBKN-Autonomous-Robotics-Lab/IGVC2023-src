...
/**
 * Map_grids rely on costmaps to identify obstacles. We need a costmap that we
 * can easily manipulate for unit tests. This class has a grid map where we can
 * set grid cell state, and a synchronize method to make the costmap match.
 */
class WavefrontMapAccessor : public costmap_2d::Costmap2D {
public:
  WavefrontMapAccessor(MapGrid *map, double outer_radius)
      : costmap_2d::Costmap2D(map->size_x_, map->size_y_, 1, 0, 0), map_(map),
