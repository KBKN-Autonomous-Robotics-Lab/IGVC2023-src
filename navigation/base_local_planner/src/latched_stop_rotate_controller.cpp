 ...
 * Meaning we might have overshot on the position beyond tolerance, yet still
 * return true. Also goal orientation might not yet be true
 */
...
/**
 * returns true if we have passed the goal position and have reached goal
 * orientation. Meaning we might have overshot on the position beyond tolerance,
 * yet still return true.
 */
bool LatchedStopRotateController::isGoalReached(
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    double angle = base_local_planner::getGoalOrientationAngleDifference(
        global_pose, goal_th);
    // check to see if the goal orientation has been reached
    if (fabs(angle) <= limits.yaw_goal_tolerance) {
      // make sure that we're actually stopped before returning success
      if (base_local_planner::stopped(base_odom, theta_stopped_vel,
                                      trans_stopped_vel)) {
        return true;
      }
    }
...
bool LatchedStopRotateController::stopWithAccLimits(
    ...
    Eigen::Vector3f acc_lim, double sim_period,
  ...
  bool valid_cmd =
      obstacle_check(Eigen::Vector3f(global_pose.pose.position.x,
                                     global_pose.pose.position.y, yaw),
                     Eigen::Vector3f(robot_vel.pose.position.x,
                                     robot_vel.pose.position.y, vel_yaw),
                     Eigen::Vector3f(vx, vy, vth));

  // if we have a valid command, we'll pass it on, otherwise we'll command all
  // zeros
  if (valid_cmd) {
    ROS_DEBUG_NAMED("latched_stop_rotate",
                    "Slowing down... using vx, vy, vth: %.2f, %.2f, %.2f", vx,
                    vy, vth);
    cmd_vel.linear.x = vx;
    cmd_vel.linear.y = vy;
...
bool LatchedStopRotateController::rotateToGoal(
  ...
  double v_theta_samp = std::min(
      limits.max_vel_theta, std::max(limits.min_vel_theta, fabs(ang_diff)));
  ...
  v_theta_samp = std::min(limits.max_vel_theta,
                          std::max(limits.min_vel_theta, v_theta_samp));
...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(
    geometry_msgs::Twist &cmd_vel, Eigen::Vector3f acc_lim, double sim_period,
    LocalPlannerUtil *planner_util, OdometryHelperRos &odom_helper_,
    const geometry_msgs::PoseStamped &global_pose,
    boost::function<bool(Eigen::Vector3f pos, Eigen::Vector3f vel,
                         Eigen::Vector3f vel_samples)>
        obstacle_check) {
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    if (!rotating_to_goal_ &&
        !base_local_planner::stopped(base_odom, limits.theta_stopped_vel,
