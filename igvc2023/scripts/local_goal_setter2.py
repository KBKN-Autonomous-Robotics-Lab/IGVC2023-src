...
class LocalGoalSetter:
    def __init__(self):
        self.costmap_sub = rospy.Subscriber(
            "move_base/local_costmap/costmap", OccupancyGrid, self.callback
        )
...
"""
        ...
        if self.init_flag:
            ...
            direction_num = 9
            self.refer_idx, self.theta = self.setReferenceIndex(
                width, height, min_angle, max_angle, direction_num
            )
        ...
        try:
            ...
            tf = self.tfBuffer.lookup_transform(
                "map", "base_footprint", stamp, rospy.Duration(1.0)
            )
        except (
        ...
        rotmat = cv2.getRotationMatrix2D(
            center=(width / 2, height / 2), angle=math.degrees(-yaw), scale=1
        )
        ...
        scores = np.zeros(
            len(self.theta), dtype=np.float32
        )  # distance to obstacle(cost) from center
        score = 0.0
        sid = 0
        window = 6
        for i in range(1, len(self.refer_idx[0])):
            ...
            if r == -1 and c == -1:
                if scores[sid] == 0:
                    scores[sid] = (
                        np.linalg.norm(
                            [
                                center[1] - self.refer_idx[0][i - 1],
                                center[0] - self.refer_idx[1][i - 1],
                            ]
                        )
                        - abs(self.theta[sid]) * 2
                    )
                sid += 1
                continue
            ...
            if (
                np.sum(rot_costmap[r - window : r + window, c - window : c + window])
                > 0
            ):
        ...
        if (
            max_idx > consider_num - 1 and max_idx < len(scores) - consider_num
        ):  # consider front and rear angles scores
            max_score = np.average(
                scores[max_idx - consider_num : max_idx + consider_num + 1]
            )
            weight = [
                (w / max_score) ** 3
                for w in scores[max_idx - consider_num : max_idx + consider_num + 1]
            ]  # to increase score difference, **3
            max_score_theta = np.average(
                self.theta[max_idx - consider_num : max_idx + consider_num + 1],
                weights=weight,
            )
        else:
        ...
        if abs(max_score_theta - self.pre_theta) > math.pi / 4:
            rospy.loginfo(
                "Local goal changed significantly compared to previous step: %s",
                abs(max_score_theta - self.pre_theta),
            )
        ...
        resolution = (
            msg.info.resolution
        )  # multiplie this value to convert to real distance
        robot_pose = [tf.transform.translation.x, tf.transform.translation.y, yaw]
        goal_msg = self.newGoalMsg(
            local_goal_x * resolution, local_goal_y * resolution, th, robot_pose
        )
        ...
        if self.select_goal_flag:
            ...
            with open(self.waypoints_file) as file:
                self.waypoints = yaml.load(
                    file
                )  # IGVC2022 has 4 pre-defined gps waypoint
            for i in range(len(self.waypoints["waypoints"])):
                self.dist.append(self.calc_dist(robot_pose, i, 0))
            self.dist.append(
                self.calc_dist(robot_pose, 0, 1)
            )  # 5th in the list is the distance from the origin

            if (
                self.dist.index(min(self.dist)) == 0
            ):  # if the robot is close to waypoint1
                self.waypoints_goal = 0  # origin
            elif (
                self.dist.index(min(self.dist)) == 1
            ):  # if the robot is close to waypoint2
                self.waypoints_goal = 3  # waypoint3
            elif (
                self.dist.index(min(self.dist)) == 2
            ):  # if the robot is close to waypoint3
                self.waypoints_goal = 2  # waypoint2
            elif (
                self.dist.index(min(self.dist)) == 3
            ):  # if the robot is close to waypoint4
                self.waypoints_goal = 0  # origin
            elif (
                self.dist.index(min(self.dist)) == 4
            ):  # if the robot is close to origin
        ...
        if self.waypoints_goal == 0:
            print(
                ...
                + "\033[34m"
                + "Distance from goal: "
                + str(self.calc_dist(robot_pose, 0, 2))
                + "\033[0m",
                end="",
            )
            ...
            if self.calc_dist(robot_pose, 0, 0) < self.calc_dist(robot_pose, 3, 0):
                print(
                    "\r"
                    + "\033[34m"
                    + "Distance from waypoint1: "
                    + str(self.calc_dist(robot_pose, 0, 0))
                    + "\033[0m",
                    end="",
                )
            else:
                print(
                    "\r"
                    + "\033[34m"
                    + "Distance from waypoint4: "
                    + str(self.calc_dist(robot_pose, 3, 0))
                    + "\033[0m",
                    end="",
                )
            ...
            print(
                "\r"
                + "\033[34m"
                + "Distance from waypoint2: "
                + str(self.calc_dist(robot_pose, 1, 0))
                + "\033[0m",
                end="",
            )
            ...
            print(
                "\r"
                + "\033[34m"
                + "Distance from waypoint3: "
                + str(self.calc_dist(robot_pose, 2, 0))
                + "\033[0m",
                end="",
            )
...
"""
            ...
            return
        elif self.waypoints_goal == 1 and (
            self.calc_dist(robot_pose, 0, 0) < distance
            or self.calc_dist(robot_pose, 3, 0) < distance
        ):
            print("")
            ...
            self.waypoints_goal == 2
            and node_flag == 1
            and self.calc_dist(robot_pose, 1, 0) < distance
        ):
            print("")
            print("\033[32m" + "Activate next node!!" + "\033[0m")
            # rospy.set_param("node_flag", 2) # node_flag == 2 then activate gps_goal_setter node
            ...
            self.waypoints_goal == 3
            and node_flag == 1
            and self.calc_dist(robot_pose, 2, 0) < distance
        ):
            print("")
            print("\033[32m" + "Activate next node!!" + "\033[0m")
            node_index += 1  # +
        ...
        rgb_costmap = cv2.circle(
            rgb_costmap,
            (center[0] - int(local_goal_y), height - (center[1] + int(local_goal_x))),
            5,
            (0, 0, 100),
            thickness=-1,
        )
    ...
    def calc_dist(self, robot_pose, num, stat):
        if stat == 0:
            return math.sqrt(
                (robot_pose[0] - self.waypoints["waypoints"][num]["point"]["x"]) ** 2
                + (robot_pose[1] - self.waypoints["waypoints"][num]["point"]["y"]) ** 2
            )
        elif stat == 1:
