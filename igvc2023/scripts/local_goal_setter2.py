...
class LocalGoalSetter:
    def __init__(self):
        ...
        self.costmap_pub = rospy.Publisher("costmap_iamge", Image, queue_size=1)
...
"""
        ...
        #-----Frequency control-----
        self.run_counter += 1
        if self.run_counter != 2: return
        else: self.run_counter = 0
        ...
        scores = np.zeros(len(self.theta), dtype=np.float16) # distance to obstacle(cost) from center
        score = 0.0
        sid = 0
        window = 6
        for i in range(1, len(self.refer_idx[0])):
            ...
            if r==-1 and c==-1:
                if scores[sid] == 0:
                    scores[sid] = np.linalg.norm([center[1]-self.refer_idx[0][i-1], center[0]-self.refer_idx[1][i-1]])
                sid+=1
                continue
...
"""
        ...
        self.costmap_pub.publish(self.cv_bridge.cv2_to_imgmsg((rgb_costmap/100*255).astype(np.uint8)))
