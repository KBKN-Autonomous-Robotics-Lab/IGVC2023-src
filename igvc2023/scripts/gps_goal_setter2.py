...
class GPSGoalSetter:
  ...
  """
    ...
    def pub_goal(self, waypoints, num):
        ...
        (roll, pitch, yaw) = euler_from_quaternion(
            [coordinates["rx"], coordinates["ry"], coordinates["rz"], coordinates["rw"]]
        )
        roll = pitch = 0.0
  ...
  """
    ...
    def select_first_goal(self, waypoints):
        ...
        for i in range(len(waypoints["waypoints"])):
            dist.append(
                math.sqrt(
                    (coordinates["x"] - waypoints["waypoints"][i]["point"]["x"]) ** 2
                    + (coordinates["y"] - waypoints["waypoints"][i]["point"]["y"]) ** 2
                )
            )
  ...
  """
    ...
    def goal_arrival_flag(self, waypoints, num, distance):
        coordinates = self.get_coordinate_from_tf(self.now)
        dist = math.sqrt(
            (coordinates["x"] - waypoints["waypoints"][num - 1]["point"]["x"]) ** 2
            + (coordinates["y"] - waypoints["waypoints"][num - 1]["point"]["y"]) ** 2
        )
        print(
            "\r"
            + "\033[34m"
