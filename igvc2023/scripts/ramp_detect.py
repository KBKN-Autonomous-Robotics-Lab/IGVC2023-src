...
class RampDetector:
    ...
    def imuCallback(self, msg):
        (_, self.pitch, _) = euler_from_quaternion(
    ...
    def scanCallback(self, msg):
        ...
        if node_flag == 3:  # Only ramp detection. When detected, set node_flag to 4
            ...
            node_flag == 4
        ):  # approach, climb, and down ramp. When finished, set node_flag to 1]
    ...
    def approachToRamp(self, scan_msg):
        ...
        try:
            ...
            tf = self.tfBuffer.lookup_transform(
                "map", "base_link", stamp, rospy.Duration(1.0)
            )
        except (
        ...
        d = np.linalg.norm(
            [
                robot_pose[0] - goal_msg.pose.position.x,
                robot_pose[1] - goal_msg.pose.position.y,
            ]
        )
    ...
    def scan2image(self, scan_msg):
        ...
        for i in range(0, len(scan_msg.ranges)):
			...
			"""
            r = (
                self.scan_img_size[0]
                - 1
                - int(math.cos(th) * scan_msg.ranges[i] * self.scale)
            )
