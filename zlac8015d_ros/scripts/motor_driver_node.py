...
class MotorDriverNode:
    def __init__(self):
        self.client = ModbusClient(
            method="rtu",
            port=rospy.get_param("/motor_driver_node/port"),
            baudrate=115200,
            timeout=1,
        )
        self.client.connect()
        ...
        rospy.Subscriber(
            rospy.get_param("/motor_driver_node/twist_cmd_vel_topic"),
            Twist,
            self.twist_cmd_callback,
        )
        rospy.Subscriber(
            rospy.get_param("/motor_driver_node/cmd_vel_topic"),
            Float32MultiArray,
            self.vel_cmd_callback,
        )
        rospy.Subscriber(
            rospy.get_param("/motor_driver_node/cmd_rpm_topic"),
            Float32MultiArray,
            self.rpm_cmd_callback,
        )
        rospy.Subscriber(
            rospy.get_param("/motor_driver_node/cmd_deg_topic"),
            Float32MultiArray,
            self.deg_cmd_callback,
        )
        rospy.Subscriber(
            rospy.get_param("/motor_driver_node/cmd_dist_topic"),
            Float32MultiArray,
        ...
        self.callback_timeout = float(
            rospy.get_param("/motor_driver_node/callback_timeout")
        )
        self.wheels_base_width = float(
            rospy.get_param("/motor_driver_node/wheels_base_width")
        )
        self.left_wheel_radius = float(
            rospy.get_param("/motor_driver_node/left_wheel_radius")
        )
        self.right_wheel_radius = float(
            rospy.get_param("/motor_driver_node/right_wheel_radius")
        )
        self.computation_left_wheel_radius = float(
            rospy.get_param("/motor_driver_node/computation_left_wheel_radius")
        )
        self.computation_right_wheel_radius = float(
            rospy.get_param("/motor_driver_node/computation_right_wheel_radius")
        )
    ...
    def speed_mode_init(self):
        ...
        result = self.client.write_registers(
            self.L_ACL_TIME, [AL_ms, AR_ms], unit=self.ID
        )
        ...
        result = self.client.write_registers(
            self.L_DCL_TIME, [DL_ms, DR_ms], unit=self.ID
        )
    ...
    def position_mode_init(self):
        ...
        result = self.client.write_registers(
            self.L_MAX_RPM_POS, [max_L_rpm, max_R_rpm], unit=self.ID
        )
        ...
        while True:
            ...
            if self.estop:
                ...
                if self.control_mode == 3:
                    ...
                    result = self.client.write_register(
                        self.CONTROL_REG, self.EMER_STOP, unit=self.ID
                    )
                ...
                if estop_reset_flag:
                    ...
                    # -----Enable Motor-----
                    result = self.client.write_register(
                        self.CONTROL_REG, self.ENABLE, unit=self.ID
                    )
            ...
            if self.debug:
                print(
                    "\033[32m"
                    + "x: {:f} | y: {:f} | yaw: {:f}".format(x, y, np.rad2deg(theta))
                    + "\033[0m"
                )
            else:
                pass
