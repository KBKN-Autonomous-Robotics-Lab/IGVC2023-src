...
class LocalGoalSetter:
	def __init__(self):
		...
		self.published_goal = PoseStamped()
		self.pre_front_score = 1
...
"""
		...
		#-----Only first callback-----
		if self.init_flag:
			min_angle = -math.pi/2 + np.deg2rad(30)
			max_angle = math.pi/2 - np.deg2rad(30)
			direction_num = 9
			self.refer_idx, self.theta = self.setReferenceIndex(width, height, min_angle, max_angle, direction_num)
			goal_msg = self.newGoalMsg(3,0,0,[0,0,0])
			self.goal_pub.publish(goal_msg)
			self.published_goal = goal_msg
		...
		reached = False
		look_forward = False
		if np.linalg.norm([robot_pose[0]-self.published_goal.pose.position.x, robot_pose[1]-self.published_goal.pose.position.y]) > 1.5:
			(_, _, goal_yaw) = euler_from_quaternion([self.published_goal.pose.orientation.x,self.published_goal.pose.orientation.y,self.published_goal.pose.orientation.z,self.published_goal.pose.orientation.w])
			"""
			if abs(robot_pose[2]-goal_yaw)<0.5:
				look_forward = True
			else:
				return
			"""
		...
		#-----Search best direction or look forward-----
		...
		for i in range(1, len(self.refer_idx[0])):
			...
			if r==-1 and c==-1:
				if scores[sid] == 0:
					scores[sid] = np.linalg.norm([center[1]-self.refer_idx[0][i-1], center[0]-self.refer_idx[1][i-1]]) - abs(self.theta[sid])
				sid+=1
				continue
		...
		center_id = int(len(self.theta)/2)
		front_score = np.average(scores[center_id-2:center_id+3])
		if (not reached) and (front_score/self.pre_front_score > 0.9):
		...
		rospy.loginfo("\nGet new goal")
		self.pre_front_score = front_score
		consider_num = 1
...
"""
		...
		if self.time_counter[1] == 30:
			print("\ntime:", self.time_counter[0]/30*1000, "[ms]")
			self.time_counter = [0.,0]
		"""
